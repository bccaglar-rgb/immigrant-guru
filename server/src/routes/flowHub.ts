/**
 * Flow Mode Hub — API Routes
 */

import type { Express } from "express";
import type { FlowModeHub } from "../engines/flowModeHub/hubOrchestrator.ts";
import { readFlowSnapshot } from "../engines/flowModeHub/hubPublisher.ts";
import { pool } from "../db/pool.ts";

export function registerFlowHubRoutes(app: Express, hub?: FlowModeHub | null): void {
  app.get("/api/flow-hub/snapshot", async (_req, res) => {
    try {
      let raw: { cycleId: string; publishedAt: number; count: number; outputs: any[] } | null = null;
      const redisSnapshot = await readFlowSnapshot();
      if (redisSnapshot) {
      ...
      res.status(500).json({ error: "Failed to read flow hub snapshot" });
    }
  });

  app.get("/api/flow-hub/history", async (req, res) => {
    try {
      ...
      const { rows } = await pool.query(
        `SELECT id, symbol, cycle_id, adjusted_score, decision, direction, regime, bias_score, core_score, edge_r, penalty, gates_passed, failed_gates, created_at
         FROM flow_hub_snapshots ORDER BY created_at DESC LIMIT $1`,
        [limit],
      );
      res.json(rows);
    } catch {
  ...
  app.get("/api/flow-hub/detail/:symbol", async (req, res) => {
    try {
      ...
      const { rows } = await pool.query(
        `SELECT full_payload FROM flow_hub_snapshots WHERE symbol = $1 ORDER BY created_at DESC LIMIT 1`,
        [symbol],
      );
  ...
  app.get("/api/flow-hub/health", async (_req, res) => {
    try {
      ...
      const redisData = await readFlowSnapshot();
      if (redisData) {

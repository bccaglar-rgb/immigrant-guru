/**
 * Aggressive Mode Hub — API Routes
 */

import type { Express } from "express";
import type { AggModeHub } from "../engines/aggModeHub/hubOrchestrator.ts";
import { readAggSnapshot } from "../engines/aggModeHub/hubPublisher.ts";
import { pool } from "../db/pool.ts";

export function registerAggHubRoutes(app: Express, hub?: AggModeHub | null): void {
  app.get("/api/agg-hub/snapshot", async (_req, res) => {
    try {
      let raw: { cycleId: string; publishedAt: number; count: number; outputs: any[] } | null = null;
      const redisSnapshot = await readAggSnapshot();
      if (redisSnapshot) {
      ...
      res.status(500).json({ error: "Failed to read agg hub snapshot" });
    }
  });

  app.get("/api/agg-hub/history", async (req, res) => {
    try {
      ...
      const { rows } = await pool.query(
        `SELECT id, symbol, cycle_id, adjusted_score, decision, direction, regime, bias_score, core_score, edge_r, penalty, gates_passed, failed_gates, created_at
         FROM agg_hub_snapshots ORDER BY created_at DESC LIMIT $1`,
        [limit],
      );
      res.json(rows);
    } catch {
  ...
  app.get("/api/agg-hub/detail/:symbol", async (req, res) => {
    try {
      ...
      const { rows } = await pool.query(
        `SELECT full_payload FROM agg_hub_snapshots WHERE symbol = $1 ORDER BY created_at DESC LIMIT 1`,
        [symbol],
      );
  ...
  app.get("/api/agg-hub/health", async (_req, res) => {
    try {
      ...
      const redisData = await readAggSnapshot();
      if (redisData) {

...
export async function LockedContent({
  heading,
  teaser,
  ctaHref,
  ctaLabel
    ...
    <section className="mt-12 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-pink-500/10 p-6 sm:p-8">
      ...
      <div className="relative mt-6 rounded-2xl border border-white/10 bg-black/40 p-6">
        <div className="space-y-2 text-white/40 [filter:blur(5px)] select-none">
          <p>{t("Plan A detailed breakdown — strongest evidence categories, filing risks, timing")}</p>
          <p>{t("Plan B alternate pathway with cost-adjusted comparison")}</p>
          <p>{t("Plan C fallback strategy — documentation needs, required experience")}</p>
          <p>{t("Personalized probability scoring across 12 dimensions")}</p>
        </div>
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-center">
          ...
          <p className="max-w-sm text-xs text-white/60">
            {t("Free to start — no card required — unlock your readiness score, top 3 plans, and next actions")}
          </p>
        </div>
      </div>
    </section>

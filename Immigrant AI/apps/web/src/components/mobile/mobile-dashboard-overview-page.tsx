  ...
  if (status === "error") {
    return (
      <DashboardErrorState
        message={error}
        onRetry={refresh}
        title={t("The mobile command center could not be loaded")}
      />
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <DashboardSignalListCard
        eyebrow={t("Top risks")}
        emptyCopy={t("No elevated operational risks are currently blocking the leading case")}
      ...
      <DashboardSignalListCard
        ...
        emptyCopy={t("No material information gaps are blocking the current recommendation")}
        ...
        <MobileCardList
          ...
          emptyState={
            <div className="rounded-2xl border border-dashed border-line bg-canvas/70 px-4 py-6 text-sm leading-7 text-muted">
              {t("Create your first case to unlock the mobile command center")}
            </div>
          }
            ...
            <Link href={`/dashboard/cases/${item.id}`}>
              <div className="rounded-2xl border border-line bg-white px-4 py-4 shadow-card">
                ...
                <p className="mt-2 text-sm text-muted">
                  {item.target_program || t("Pathway not set")}
                </p>
              </div>
            </Link>
          )}

...
export function UserDetailDrawer({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteError(err instanceof Error ? err.message : t("Delete failed"));
      setIsDeleting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <aside className="flex h-full w-full max-w-xl flex-col overflow-y-auto border-l border-line bg-white shadow-2xl">
        ...
        <div className="flex-1 space-y-6 px-6 py-6">
          ...
          {user.profile ? (
            <section>
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                ...
                <Field label={t("English level")}>
                  <span className="text-xs text-ink">{user.profile.english_level ?? "—"}</span>
                </Field>
                <Field label={t("Experience")}>
                  <span className="text-xs text-ink">{user.profile.years_of_experience !== null ? `${user.profile.years_of_experience} ${t("yrs")}` : "—"}</span>
                </Field>
            ...
            <section>
              ...
              <p className="mt-3 rounded-xl border border-dashed border-line bg-canvas/40 px-4 py-6 text-center text-xs text-muted">
                {t("User has not completed onboarding")}
              </p>
            </section>
          )}
          ...
          <section>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted">
              {t("Immigration cases")} ({user.immigration_cases.length})
            </p>
                ...
                user.immigration_cases.map((c) => (
                  <div key={c.id} className="rounded-xl border border-line bg-canvas/40 p-3">
                    <div className="flex items-baseline justify-between gap-3">
                      <p className="truncate text-sm font-semibold text-ink">{c.title || t("Untitled case")}</p>
                    ...
                    <p className="mt-1 font-mono text-[10px] text-muted">{c.id.slice(0, 8)} · {t("updated")} {fmtDate(c.updated_at)}</p>
                  </div>
                ))
              )}
            </div>
        ...
        {confirmDelete && (
          <div className="border-t border-line bg-red/5 px-6 py-5">
            <p className="text-sm font-semibold text-red">{t("Delete this account")}</p>
            <p className="mt-1 text-xs text-muted">
              {t("This permanently removes")} <span className="font-medium text-ink">{user.email}</span> {t("and all associated data — this cannot be undone")}
            </p>
            {deleteError && (

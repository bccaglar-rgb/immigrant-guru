...
export function DashboardTimelinePreviewCard({
  ...
  return (
    <DashboardCommandCard
      eyebrow={t("Timeline preview")}
      title={data.nextStep || t("Timeline planning pending")}
      value={
        <div>
    ...
    >
      <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-2xl bg-slate-50/80 px-4 py-4">
          ...
          <p className="mt-2 text-sm font-semibold text-ink">
            {data.nextStep || t("Open a case workspace to estimate the next stage")}
          </p>
          <p className="mt-2 text-sm leading-6 text-muted">
            {data.nextStepDurationMonths === null
              ? t("A step duration will appear once the active case has enough pathway context")
              : `${t("Expected preparation time")}: ${data.nextStepDurationMonths.toFixed(1)} ${t("months")}`}
          </p>
        </div>
        <div className="rounded-2xl bg-blue-50/80 px-4 py-4">
          ...
          <p className="mt-2 text-sm leading-6 text-muted">
            {data.accelerationTips[0] ||
              t("Parallelize profile cleanup and document collection to shorten the first preparation phase")}
          </p>
        </div>
      </div>
      <div className="mt-4 rounded-2xl border border-amber-200/80 bg-amber-50/70 px-4 py-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted">
          {data.delayRisks[0] ||
            t("No elevated delay signal is surfaced yet, but timeline precision improves with stronger case data")}
        </p>
      </div>
    </DashboardCommandCard>
  );

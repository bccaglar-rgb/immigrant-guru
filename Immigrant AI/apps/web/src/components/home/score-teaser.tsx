...
export function ScoreTeaser() {
  ...
  const scoreSignals = [
    t("Profile depth and readiness"),
    t("Target-country fit"),
    t("Program alignment"),
    t("Risk and preparation gaps")
  ];
  ...
  return (
    <SectionContainer
      ...
      description={t(
        "Deterministic scoring combined with case and profile signals so you can understand readiness without guesswork"
      )}
      eyebrow={t("Immigration Score")}
      title={t("Explainable readiness signals")}
    >
      <div className="grid gap-6 lg:grid-cols-2" id="score">
        <Animate animation="slide-left" duration={800}>
          <div className="glass-card rounded-3xl p-8 md:p-10 h-full">
            <p className="text-sm font-semibold text-accent">{t("Score Preview")}</p>
            <div className="mt-6 flex items-end gap-2">
              <span className="text-7xl font-semibold tracking-tight text-ink md:text-8xl anim-count">
                72
              </span>
            ...
            <p className="mt-4 text-sm leading-relaxed text-muted">
              {t(
                "Scores combine profile completeness, professional strength, financial readiness, and case readiness into a transparent signal you can improve over time"
              )}
            </p>
        ...
        <Animate animation="slide-right" duration={800} delay={150}>
          <div className="glass-card rounded-3xl p-8 md:p-10 h-full">
            <p className="text-sm font-semibold text-accent">{t("Signal Inputs")}</p>

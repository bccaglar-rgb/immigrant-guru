...
export function PathwayTeaser() {
  ...
  return (
    <SectionContainer
      ...
      description={t(
        "Surface multiple strategic routes so you're never forced into a single recommendation without context"
      )}
      eyebrow={t("Plan A / B / C")}
      title={t("Compare multiple visa strategies")}
    >
      <Stagger
        ...
        {pathways.map((pathway) => (
          ...
          >
            <div className="absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100">
              <div
                className={`absolute inset-0 bg-gradient-to-br ${pathway.gradient} opacity-[0.04]`}
              />
            </div>
            <div className="relative">
              <span
                className={`inline-flex rounded-full bg-gradient-to-r ${pathway.gradient} px-3 py-1 text-xs font-semibold text-white transition-transform duration-300 group-hover:scale-105`}
              >
                {pathway.plan}
              </span>

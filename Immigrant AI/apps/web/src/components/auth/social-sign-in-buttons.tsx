...
export function SocialSignInButtons({ nextPath, onError }: SocialSignInButtonsProps) {
  ...
  // Hide each provider when its env is missing — MVP-friendly: don't ship
  // dead buttons that can only emit "not configured" errors.
  const showGoogle = Boolean(googleClientId);
  const showApple = Boolean(appleServiceId && appleRedirectUri);

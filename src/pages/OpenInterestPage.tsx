...
export default function OpenInterestPage() {
  ...
  const oi1h = market.derivatives?.oiChange1h ?? 0;
      ...
      <div className="mx-auto max-w-[1560px] space-y-4">
        ...
        <section className="rounded-2xl border border-white/10 bg-[#121316] p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h1 className="text-lg font-semibold text-white">Open Interest</h1>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-white/10 bg-[#0F1012] p-4">
              <p className="text-xs text-[#6B6F76]">OI (1h %)</p>
              <p className={`mt-1 text-2xl font-semibold ${oi1h >= 0 ? "text-[#8fc9ab]" : "text-[#d49f9a]"}`}>{oi1h.toFixed(2)}%</p>
            </div>
          </div>
        </section>
      </div>

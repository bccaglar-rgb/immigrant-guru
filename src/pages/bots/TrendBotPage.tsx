...
const TRUST_STATS = [
  ...
  { label: "Max DD", value: "-8.1%", color: "#f6465d" },
  { label: "Trades", value: "142", color: "#fff" },
  { label: "Performance", value: "+8.2%", color: "#2bc48a" },
];

/* ── Bot thinking conditions ── */
const CONDITIONS = [
  { label: "EMA9 > EMA21", met: true, detail: "94,500 > 94,200" },
  { label: "ADX > 20", met: true, detail: "28" },
  { label: "Volume > 1.2x avg", met: false, detail: "0.9x" },
  { label: "Trend sustained", met: true, detail: "4 candles" },
];

/* ── Strategy logic conditions ── */
const ENTRY_LONG = [
  { rule: "EMA9 > EMA21", met: true },
  { rule: "ADX > 20", met: true },
  { rule: "Volume > avg", met: false },
];
const ENTRY_SHORT = [
  { rule: "EMA9 < EMA21", met: false },
  { rule: "ADX > 20", met: true },
  { rule: "Volume > avg", met: false },
];
        ...
        <Card>
          ...
          <div className="mt-4 rounded-lg bg-white/[0.03] p-3">
            ...
            <p className="mt-1 text-[12px] font-medium text-white/70">
              Trend confirmed, waiting for volume spike
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="flex flex-col gap-1">
              ...
              <input
                type="number"
                value={setup.emaFast}
                onChange={(e) => setSetup({ ...setup, emaFast: +e.target.value })}
                className="rounded-md border border-white/10 bg-[#0F1012] px-3 py-1.5 text-xs text-white outline-none"
              />
            </label>
            ...
            <label className="flex flex-col gap-1">
              ...
              <input
                type="number"
                value={setup.emaSlow}
                onChange={(e) => setSetup({ ...setup, emaSlow: +e.target.value })}
                className="rounded-md border border-white/10 bg-[#0F1012] px-3 py-1.5 text-xs text-white outline-none"
              />
            </label>
        ...
        <Card>
          ...
          <div className="space-y-3">
            {[
              ...
              { label: "Notional Value", value: `$${fmt(setup.size * 94500 * setup.leverage, 0)}`, color: "#5B8DEF" },
            ].map((r) => (
              <div
                key={r.label}
            ...
            <div className="mt-2 rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
              ...
              <div className="mt-2 flex items-center gap-3">
                ...
                <span className="text-[11px] text-white/40">
                  Volatility: <span className="font-medium text-white/70">1.8% (24h)</span>
                </span>
              </div>
            </div>
          </div>
      ...
      <Card>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
            <p className="mb-2 text-[11px] font-semibold text-[#2bc48a]">Entry Long</p>
          ...
          {/* Entry Short */}
          <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
            <p className="mb-2 text-[11px] font-semibold text-[#f6465d]">Entry Short</p>
            <div className="space-y-1.5">
              {ENTRY_SHORT.map((c) => (
                <div key={c.rule} className="flex items-center gap-2 text-[11px]">
                  <span>{c.met ? "\u2705" : "\u274C"}</span>
          ...
          <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-3">
            ...
            <div className="space-y-1.5 text-[11px] text-white/60">
              <p>TP: 2x risk</p>
              <p>SL: Recent swing high/low</p>
              <p>Trailing stop active</p>
            </div>
          </div>
        </div>
      </Card>
            ...
            {[
              {
                label: "Status",
                render: (
                  <span className="rounded-md bg-[#2bc48a]/15 px-2 py-0.5 text-[11px] font-bold text-[#2bc48a]">
                    READY
                  </span>
                ),
              },
              ...
              },
              {
                ...
                render: (
                  <span className="text-[12px] text-white/60">
                    Scanning for volume confirmation
                  </span>
                ),
              },
            ].map((row) => (

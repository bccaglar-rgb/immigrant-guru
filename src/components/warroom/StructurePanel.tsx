...
export const StructurePanel = ({ data }: Props) => (
  <div className="rounded-2xl border border-white/[0.05] bg-white/[0.02] p-3 space-y-2">
    ...
    <div className="space-y-0.5">
      <span className="text-[8px] font-bold text-[#f6465d] uppercase">Resistance</span>
      {data.resistances.map((l, i) => <LevelRow key={i} level={l} color="#f6465d" />)}
    </div>

    {/* Supports */}
    <div className="space-y-0.5">
      <span className="text-[8px] font-bold text-[#2bc48a] uppercase">Support</span>
    ...
    <div className="flex items-center justify-between rounded-lg bg-[#f6465d]/[0.06] px-2.5 py-1.5">
      <span className="text-[8px] font-bold text-[#f6465d]">INVALIDATION</span>
      <span className="font-mono text-[10px] font-bold text-[#f6465d]">${data.invalidation.toFixed(2)}</span>
    </div>

    {/* Liquidity */}
    <div className="space-y-0.5">
      <span className="text-[8px] font-bold text-[#5B8DEF] uppercase">Liquidity Zones</span>
      {data.liquidityZones.map((z, i) => (
        <div key={i} className="flex items-center justify-between py-0.5">
          ...
          <span className="text-[8px] text-[var(--textSubtle)]">{z.side.toUpperCase()} · {z.size}</span>
        </div>
      ))}
    </div>
    ...
    {/* Imbalance */}
    <div className="space-y-0.5">
      <span className="text-[8px] font-bold text-[var(--textSubtle)] uppercase">Imbalance Zones</span>
      {data.imbalanceZones.map((z, i) => (
        <div key={i} className="flex items-center justify-between py-0.5">
          <span className="font-mono text-[9px] text-[var(--textMuted)]">${z.from.toFixed(2)} — ${z.to.toFixed(2)}</span>
          <span className={`text-[8px] font-bold ${z.filled ? "text-[var(--textSubtle)]" : "text-[#F5C542]"}`}>{z.filled ? "Filled" : "Open"}</span>
        </div>
      ))}
    </div>
  </div>
...
const LevelRow = ({ level, color }: { level: Level; color: string }) => (
  <div className="flex items-center justify-between py-0.5">
    <div className="flex items-center gap-1.5">
      <span className="rounded px-1 py-px text-[7px] font-bold" style={{ color, background: `${color}15` }}>{level.type.toUpperCase()}</span>
      <span className="font-mono text-[9px] text-[var(--text)]">${level.price.toFixed(2)}</span>
    </div>
    <div className="flex items-center gap-1.5">
      <span className="text-[8px] text-[var(--textSubtle)]">{level.label}</span>

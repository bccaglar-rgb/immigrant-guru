...
export const AlertMatrix = ({ alerts }: Props) => (
  <div className="rounded-2xl border border-white/[0.05] bg-white/[0.02] p-3 space-y-2">
    <div className="flex items-center justify-between">
      ...
      <span className="rounded-full bg-[#f6465d]/10 px-1.5 py-0.5 text-[9px] font-bold text-[#f6465d]">{alerts.filter(a => a.severity === "high").length} HIGH</span>
    </div>
          ...
          <div key={i} className={`flex items-start gap-2 rounded-lg border-l-2 bg-black/20 px-2.5 py-1.5 ${s.border}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="rounded bg-white/[0.06] px-1 py-px text-[9px] font-bold text-[var(--textSubtle)]">{typeIcon[a.type]}</span>

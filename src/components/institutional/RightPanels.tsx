...
export const BTCChart = ({ data }: { data: C[] }) => {
  const cvs = useRef<HTMLCanvasElement>(null), box = useRef<HTMLDivElement>(null);
  const last = data[data.length - 1], prev = data[data.length - 2];
  const pct = prev ? ((last.close - prev.close) / prev.close) * 100 : 0, up = pct >= 0;
  const { mn, mx } = useMemo(() => { let a = Infinity, b = -Infinity; for (const d of data) { if (d.low < a) a = d.low; if (d.high > b) b = d.high; } const p = (b - a) * 0.08; return { mn: a - p, mx: b + p }; }, [data]);
  ...
  useEffect(() => {
    ...
    const rng = mx - mn || 1, cw = Math.max(1, (w - 4) / data.length), bw = Math.max(1, cw * 0.45);
    for (let i = 0; i < data.length; i++) {
      const d = data[i], px = 2 + i * cw + cw / 2;
      const yH = ((mx - d.high) / rng) * h, yL = ((mx - d.low) / rng) * h, yO = ((mx - d.open) / rng) * h, yC = ((mx - d.close) / rng) * h;
  ...
  return (
    <div className="rounded-2xl border border-white/[0.04] bg-white/[0.015] overflow-hidden">
      <div className="flex items-center justify-between px-2.5 py-1 border-b border-white/[0.04]">
        <div className="flex items-center gap-1.5"><span className="text-[9px] font-bold text-[var(--text)]">BTC/USDT</span><span className="rounded bg-white/[0.05] px-1 py-px font-mono text-[7px] text-[var(--textMuted)]">1m</span></div>
        <div className="flex items-center gap-1.5">
          <span className={`font-mono text-[9px] font-bold ${up ? "text-[#2bc48a]" : "text-[#f6465d]"}`}>${last?.close.toLocaleString()}</span>
          <span className={`rounded-full px-1 py-px font-mono text-[7px] font-bold ${up ? "bg-[#2bc48a]/10 text-[#2bc48a]" : "bg-[#f6465d]/10 text-[#f6465d]"}`}>{up ? "+" : ""}{pct.toFixed(2)}%</span>
        </div>
      </div>
      <div ref={box} className="h-20"><canvas ref={cvs} className="h-full w-full" /></div>
    </div>
  );
};
  ...
  return (
    <div className="rounded-2xl border border-white/[0.04] bg-white/[0.015] p-2.5 space-y-1.5">
      ...
      <div className="rounded-lg border border-[#F5C542]/10 bg-[#F5C542]/[0.02] px-2 py-1.5">
        <span className="text-[6px] font-bold text-[#F5C542] uppercase tracking-wider">AI Narrative</span>
        <p className="mt-0.5 text-[8px] leading-relaxed text-[var(--textMuted)]">{intel.narrative}</p>
      </div>
    </div>
  );
};
...
export const StructureLevelsPanel = ({ data }: LvlProps) => (
  <div className="rounded-2xl border border-white/[0.04] bg-white/[0.015] p-2.5 space-y-1.5">
    ...
    <div className="space-y-0.5"><span className="text-[7px] font-bold text-[#f6465d] uppercase">Resistance</span>
      {data.resistances.map((l, i) => <LR key={i} p={l.price} lbl={l.label} s={l.strength} c="#f6465d" htf={l.htf} />)}</div>
    <div className="space-y-0.5"><span className="text-[7px] font-bold text-[#2bc48a] uppercase">Support</span>
    ...
    <div className="flex items-center justify-between rounded-lg bg-[#f6465d]/[0.05] px-2 py-1">
      <span className="text-[7px] font-bold text-[#f6465d]">INVALIDATION</span>
    ...
    <div className="space-y-0.5"><span className="text-[7px] font-bold text-[#5B8DEF] uppercase">Liquidity</span>
      {data.liquidityZones.map((z, i) => (
        <div key={i} className="flex items-center justify-between py-px">
          <div className="flex items-center gap-1"><span className={`h-1 w-1 rounded-full ${z.side === "buy" ? "bg-[#2bc48a]" : "bg-[#f6465d]"}`} /><span className="font-mono text-[8px] text-[var(--text)]">${z.price}</span></div>
          <span className="text-[7px] text-[var(--textSubtle)]">{z.side.toUpperCase()} · {z.magnitude}</span>
        </div>
      ))}
    </div>
    <div className="space-y-0.5"><span className="text-[7px] font-bold text-[var(--textSubtle)] uppercase">Imbalances</span>
      {data.imbalances.map((z, i) => (
        <div key={i} className="flex items-center justify-between py-px">
          <span className="font-mono text-[8px] text-[var(--textMuted)]">${z.from} — ${z.to}</span>
          <span className={`text-[7px] font-bold ${z.status === "Open" ? "text-[#F5C542]" : "text-[var(--textSubtle)]"}`}>{z.status}</span>
        </div>
      ))}
    </div>
  </div>
  ...
  return (
    <div className="rounded-2xl border border-white/[0.04] bg-white/[0.015] p-2.5 space-y-1.5">
      <SectionHead icon={<svg viewBox="0 0 24 24" className="h-3 w-3" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>} label="Alerts" color="#f6465d"
        right={<span className="rounded-full bg-[#f6465d]/10 px-1 py-px text-[7px] font-bold text-[#f6465d]">{alerts.filter(a => a.sev === "high").length} HIGH</span>} />
      {alerts.map((a, i) => { const s = sevS[a.sev]; return (
        <div key={i} className={`flex items-start gap-1.5 rounded-lg border-l-2 bg-black/20 px-2 py-1 ${s.bl}`}>
          ...
          <span className="flex-1 text-[8px] text-[var(--text)] truncate">{a.text}</span>
          <span className="flex-shrink-0 text-[7px] text-[var(--textSubtle)]">{a.time}</span>
        </div>
      ); })}
    </div>
  );
    ...
    <div className="rounded-2xl border border-white/[0.04] bg-white/[0.015] p-2.5 space-y-1.5">
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="space-y-1">
          <span className="text-[7px] font-bold text-[var(--accent)] uppercase">Session: {session.name}</span>
        ...
        {/* Sentiment */}
        <div className="space-y-1">
          <span className="text-[7px] font-bold text-[#8fc9ab] uppercase">Sentiment</span>
          ...
          <span className="text-[7px] text-[var(--textSubtle)]">Contrarian: {sentiment.contrarian}</span>
        </div>
        {/* Exec Quality */}
        <div className="space-y-1">
          <span className="text-[7px] font-bold text-[#5B8DEF] uppercase">Exec Quality</span>
...
const KV = ({ l, v, c }: { l: string; v: string; c?: string }) => (
  <div className="rounded bg-black/20 px-1.5 py-0.5"><div className="text-[6px] text-[var(--textSubtle)]">{l}</div><div className="text-[8px] font-bold" style={{ color: c ?? "var(--text)" }}>{v}</div></div>
);
const IB = ({ l, t }: { l: string; t: string }) => (
  <div className="rounded-lg bg-black/20 px-2 py-1"><span className="text-[6px] font-bold text-[var(--textSubtle)] uppercase">{l}</span><p className="text-[8px] text-[var(--textMuted)]">{t}</p></div>
);
const LR = ({ p, lbl, s, c, htf }: { p: number; lbl: string; s: number; c: string; htf: boolean }) => (
  <div className="flex items-center justify-between py-px">
    <div className="flex items-center gap-1"><span className="rounded px-0.5 py-px text-[6px] font-bold" style={{ color: c, background: `${c}12` }}>{htf ? "HTF" : "LTF"}</span><span className="font-mono text-[8px] text-[var(--text)]">${p.toFixed(2)}</span></div>
    <div className="flex items-center gap-1"><span className="text-[7px] text-[var(--textSubtle)]">{lbl}</span><div className="h-[3px] w-6 rounded-full bg-white/[0.05]"><div className="h-full rounded-full" style={{ width: `${s}%`, background: c }} /></div></div>
  </div>
);

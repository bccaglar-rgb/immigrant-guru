/**
 * AggHubPanel — Full diagnostic detail panel for a selected Agg Hub item
 * Orange theme to match AggHubCard
 */
import type { AggSnapshotItem } from "../../hooks/useAggHub";

/* ── Score bar helper ───────────────────────────────────── */
function ScoreBar({ label, value, max = 100 }: { label: string; value: number; max?: number }) {
  const pct = Math.min((value / max) * 100, 100);
  const color = value >= 65 ? "#f0a050" : value >= 45 ? "#d4a060" : "#d46a6a";
  return (
...
function GateChip({ name, passed }: { name: string; passed: boolean }) {
  return (
    <span className={`inline-flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[10px] font-medium ${
      passed ? "border-[#8f5f3d]/60 bg-[#201510] text-[#f0a050]" : "border-[#5a3030]/60 bg-[#1c1416] text-[#d46a6a]"
    }`}>
      <span className={`h-1.5 w-1.5 rounded-full ${passed ? "bg-[#f0a050]" : "bg-[#d46a6a]"}`} />
      {name}
    </span>
  );
}
...
function TpSlVisual({ tpSl, direction: _direction }: { tpSl: NonNullable<AggSnapshotItem["payload"]["tpSl"]>; direction: string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="text-center text-[10px] text-[#d4b890]">
...
export function AggHubPanel({ item, onClose }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-[#8f5f3d]/50 bg-[#140e08] p-3 shadow-[0_12px_36px_rgba(0,0,0,0.4)]">
      {/* Header */}
      ...
      <div className="flex items-center gap-3">
        <div className="text-center">
          <div className="text-2xl font-black text-[#f0c080]">{item.adjustedScore.toFixed(1)}</div>
          <div className="text-[10px] text-[#6B6F76]">Adjusted</div>
        </div>
        <div className={`rounded-lg border px-2.5 py-1 text-xs font-bold ${
          item.decision === "STRONG_MOMENTUM" ? "border-[#a06020] bg-[#2a1800] text-[#ff8c00]"
          : item.decision === "APPROVED" ? "border-[#8f5f3d] bg-[#251a08] text-[#f0a050]"
          : item.decision === "SCOUT" ? "border-[#6b4a20] bg-[#201a10] text-[#d4a060]"
          : item.decision === "WATCHLIST" ? "border-[#5a4a20] bg-[#1a1810] text-[#c4a060]"
        ...
        <div className="ml-auto text-right">
          ...
          <div className={`text-sm font-semibold ${
            ...
            : item.regime === "BREAKOUT_SETUP" ? "text-[#ff8c00]"
      ...
      {coreBreak && (
        <div className="space-y-1.5">
          <div className="text-[11px] font-semibold text-[#f0c080]">Core Score Breakdown</div>
          ...
          <div className="flex items-center justify-between pt-0.5 border-t border-white/5">
            ...
            <span className="text-sm font-bold text-[#f0c080]">{coreBreak.total.toFixed(1)}</span>
          </div>
        </div>
      )}
      ...
      {/* Bias Gauge */}
      <div className="space-y-1">
        <div className="text-[11px] font-semibold text-[#f0c080]">Directional Bias</div>
        <BiasGauge score={item.biasScore} />
      </div>

      {/* Gates */}
      <div className="space-y-1">
        <div className="text-[11px] font-semibold text-[#f0c080]">Hard Gates</div>
      ...
      {Object.keys(penalties).length > 0 && (
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold text-[#f0c080]">Penalties</span>
      ...
      {/* Edge Details */}
      <div className="space-y-1">
        <div className="text-[11px] font-semibold text-[#f0c080]">Expected Edge</div>
        <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px]">
          {p?.pWin != null && (
          ...
          {p?.riskAdjustedEdge != null && (
            <div className="flex justify-between"><span className="text-[#8c8e93]">Risk-Adj Edge</span>
              <span className={`font-semibold ${p.riskAdjustedEdge >= 0.2 ? "text-[#f0a050]" : p.riskAdjustedEdge >= 0.1 ? "text-[#d4a060]" : "text-[#d46a6a]"}`}>
      ...
      {tpSl && (
        <div className="space-y-1">
          <div className="text-[11px] font-semibold text-[#f0c080]">TP / SL Levels</div>
      ...
      {posSize && (
        <div className="space-y-1">
          <div className="text-[11px] font-semibold text-[#f0c080]">Position Sizing</div>
          <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[10px]">
      ...
      {p?.reasons && p.reasons.length > 0 && (
        <div className="space-y-1">
          <div className="text-[11px] font-semibold text-[#f0c080]">Reasoning</div>
